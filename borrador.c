...
void copiar(char temp[],int i){
    int N = strlen(temp) + 1;//sumo 1 porque no cuenta el final de linea
    //paso[i].accion2 = (char*)malloc(N*sizeof(char));
    // = (char*)malloc(N*sizeof(char));
    struct parametro *pthread_data = malloc(sizeof(struct parametro));

    strcpy(pthread_data->pasos_param[i].accion[i],temp);
    //strcpy(paso[i].accion2,temp);
    //strcpy(paso->accion2[i],temp);
}
...
void* ejecutarReceta(void *i) {
    ...
    //char temp [50];
    int cont = 0; //contador de lineas de la receta
    char temp[50];
    char aux;
    ...
    while (!feof(RECETA)){
        fgets(temp,50,RECETA);
        cont++;
    }

    rewind(RECETA); //vuelvo cursor al inicio
    ...
    paso = (pasos*)malloc(cont*sizeof(paso)); 




    for (l=0; !feof(RECETA);l++){
        vaciar(temp);
    ...
    //int k;
    //for (k=0;k < cont; k++){
    //    strcpy(pthread_data->pasos_param->accion[k],paso->accion2[k]);
    //    //printf(paso->accion2[k]);
    //}
